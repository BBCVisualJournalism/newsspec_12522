(function () {
    ...
    IframeWatcher.prototype = {
        ...
        createIframe: function () {
            ...
            var newhref = window.location.host + href.substring(href.indexOf('/news/special/'), href.length);
            if (href.indexOf('www.') > 1) {
                newhref = href.substring(0, href.indexOf('www.')) + newhref;
            }
            href = newhref;
            ...
            this.elm.id = 'newsspec_12522Iframe';
            ...
            // urlParams += ('&docdomain=' + document.domain);

            // this.elm.src = href + '&hostid=' + hostId.split('//')[1] + '&hostUrl=' + hostUrl + '&iframeUID=' + linkId + '&onbbcdomain=' + onBBC + urlParams;
            this.elm.src = href + '&hostid=' + hostId.split('//')[1] + '&hostUrl=' + hostUrl + '&iframeUID=' + linkId + ('&docdomain=' + document.domain) + '&onbbcdomain=' + onBBC + urlParams;
            ...
            if (window.addEventListener) {
                ...
                window.attachEvent("onscroll", this.windowScrollListener);
            }   
            ...
            // var postDomainInterval = setInterval(function () {
            //     var iframeWin = document.getElementById('newsspec_12522Iframe').contentWindow;
            //     iframeWin.postMessage('isScrollEvent', '*');
            // }, 100);
        },
        ...
        windowScrollListener: function () {
            ...
            scrollTimeout = setTimeout(function () {
                if (window.postMessage) {
                    var iframeEl = document.getElementById('newsspec_12522Iframe');
                    var parentPageScrollTop = document.documentElement.scrollTop || document.body.scrollTop;
                    var postMessagedata = {
                        'docdomain': document.domain,
                        'isScrollEvent': true,
                        'parentPageScrollTop': parentPageScrollTop,
                        'parentPageWindowHeight': 'innerHeight' in window ? window.innerHeight : document.documentElement.clientHeight,
                        'iframeTopOffset': iframeEl.getBoundingClientRect().top + (window.pageYOffset || parentPageScrollTop) - document.documentElement.clientTop

                    };

                    iframeEl.contentWindow.postMessage(JSON.stringify(postMessagedata), '*');
                }
            }, 250);
        },
        ...
        getAnyInstructionsFromIframe: function () {
            ...
            else if (this.data.msg === 'scrollToPos') {
                this.scrollToPos(this.data.targetPos);
            }
        },
        ...
        scrollToPos: function (targetIframePos) {
            var parentPageScrollTop = document.documentElement.scrollTop || document.body.scrollTop;
            var iframeTopPos = document.getElementById('newsspec_12522Iframe').getBoundingClientRect().top + (window.pageYOffset || parentPageScrollTop) - document.documentElement.clientTop;
            var targetScrollPos = iframeTopPos + targetIframePos;
